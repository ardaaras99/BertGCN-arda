...
# TF-IDF matrix
doc_word_freq = {}
row_tfidf_train, col_tfidf_train, weight_tfidf_train = [], [], []
row_tfidf_test, col_tfidf_test, weight_tfidf_test = [], [], []
...
for i, doc_words in enumerate(shuffle_doc_words_list):
    ...
    for word in words:
        ...
        if i < train_size:
            train_flag = True
            row_tfidf_train.append(i)
            col_tfidf_train.append(j)

            row.append(i)
        else:
            test_flag = True
            row_tfidf_test.append(k)
            col_tfidf_test.append(j)
        ...
        if train_flag:
            weight_tfidf_train.append(freq * idf)

        if test_flag:
            weight_tfidf_test.append(freq * idf)
...
adj_pmi = sp.csr_matrix(
    (weight_pmi, (row_pmi, col_pmi)), shape=(vocab_size, vocab_size))

adj_tfidf_train = sp.csr_matrix(
    (weight_tfidf_train, (row_tfidf_train, col_tfidf_train)), shape=(train_size, vocab_size))
adj_tfidf_test = sp.csr_matrix(
    (weight_tfidf_test, (row_tfidf_test, col_tfidf_test)), shape=(test_size, vocab_size))
