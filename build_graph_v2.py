...
for line in lines:
    ...
    if temp[1].find('test') != -1:
        test_ids.append(int(line.strip().split('\t')[0]))
    elif temp[1].find('train') != -1:
        train_ids.append(int(line.strip().split('\t')[0]))
f.close()
...
if v.pre_process_text == 'True':
    ...
    for i, doc in enumerate(doc_content_list):
        ...
        doc_filtered = [word for word in word_tokenize(
            doc_p_removed) if word not in stop_words]
...
th.save(NF, 'datas/{}/graphs/{}_NF.pt'.format(v.dataset, v.dataset))
th.save(FN, 'datas/{}/graphs/{}_FN.pt'.format(v.dataset, v.dataset))
th.save(NN, 'datas/{}/graphs/{}_NN.pt'.format(v.dataset, v.dataset))
th.save(FF, 'datas/{}/graphs/{}_FF.pt'.format(v.dataset, v.dataset))
...
objs = [(doc_content_list, 'docs'), (y, 'y'), (train_ids,
                                               'train_ids'), (test_ids, 'test_ids')]
